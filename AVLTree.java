...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
       ...
       // Move to the left subtree if the value is smaller than the current node.
        if (value < node.value) {
            // Use recursion to call 'deleteElement' on left child.
            node.leftChild = deleteElement(value, node.leftChild);
        // Move to the right subtree if the value is greater than the current node.
        } else if (value > node.value) {
            // Use recursion to call 'deleteElement' on right child
            node.rightChild = deleteElement(value, node.rightChild);
        // When the value is equal to the value of the current node, need to delete it.
        } else {
            // If the node is a leaf node with no children, delete it by returning null.
            ...
            // If the node has two children replace it with inorder successor.
            } else {
                // Smallest value in the right subtree is the inorder successor.
                Node temp = minValueNode(node.rightChild);
                // Inorder successor value is copied to temporary value.
                node.value = temp.value;
                // Inorder successor deleted from its place in the right subtree.
                node.rightChild = deleteElement(temp.value, node.rightChild);
            }
        }

        // When deleted, height of the node needs to be updated.
        node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)) + 1;
        // Check if node needs rebalancing.
        int balance = getBalanceFactor(node);
        // If the node's bf is greater than 0 complete a left-left rotation.
        if (balance > 1 && getBalanceFactor(node.leftChild) >= 0) {
            return LLRotation(node);
        // If imbalance in left child's right subtree complete a left-right rotation.
        } else if (balance > 1 && getBalanceFactor(node.leftChild) < 0) {
            return LRRotation(node);
        // If imbalance in left child's left subtree perform a right-right rotation.
        } else if (balance < -1 && getBalanceFactor(node.rightChild) <= 0) {
            return RRRotation(node);
        // If imbalance in left child's right subtree perform a right-left rotation.
        } else if (balance < -1 && getBalanceFactor(node.rightChild) > 0) {
            return RLRotation(node);
        }

        // Return the current node when the tree is balanced.
        return node;
    }
